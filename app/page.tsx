...
import { DictationPage, RacePage } from "../src/components/PracticeModes";
import { cloudEnabled, supabase } from "../src/lib/supabase";

type View = "home" | "lessons" | "trainer" | "speed" | "race" | "dictation" | "results" | "account";
...
const requiredAccuracy = (lessonId: number) => lessonId <= 4 ? 88 : lessonId <= 12 ? 90 : lessonId <= 24 ? 92 : 94;
const lessonTip = (lessonId: number) => lessonId <= 4 ? "Саусақтарды бастапқы қалыпта ұстап, пернетақтаға қарамаңыз." : lessonId <= 12 ? "Алдымен дәлдікке мән беріп, бірқалыпты ырғақты сақтаңыз." : lessonId <= 24 ? "Қазақ әріптерін тиісті саусақпен басып, сөзді тұтас оқыңыз." : "Жылдамдықты қумай, тыныс белгілері мен бас әріптерді дәл теріңіз.";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      if (active && data) {
        ...
        const cloudCompleted = cloudHistory.flatMap(result => result.lessonId && !result.title.startsWith("✎") && result.accuracy >= requiredAccuracy(result.lessonId) ? [result.lessonId] : []);
  ...
  const openWeakPractice = (letters: string[]) => { const clean = letters.length ? letters : ["ә","і","ң","ғ","ү","ұ","қ","ө","һ"], banks:Record<string,string[]>={"ә":["әке","әлем","әсем","тәлім"],"і":["ілім","білім","ізгілік","тілек"],"ң":["жаңа","таң","қоңыр","жаңғырық"],"ғ":["ғалым","ғылым","ғарыш","жаңғыру"],"ү":["үлкен","күн","күміс","бүгін"],"ұ":["ұлт","ұстаз","құдық","құрылыс"],"қ":["қазақ","қалам","құқық","ақиқат"],"ө":["өмір","өнер","көңіл","көктем"],"һ":["қаһарман","гауһар","жиһаз","шаһар"]}; const words=clean.flatMap(letter=>banks[letter.toLowerCase()]||[letter.repeat(3),`${letter}а${letter}`,`а${letter}а`,`${letter}е${letter}`]); setExercise({ title: "Қателерге бейімделген жеке жаттығу", text: words.join(" ") }); navigate("trainer"); };
  const openAdaptive = () => { const totals=history.reduce<Record<string,number>>((all,result)=>{Object.entries(result.mistakeKeys||{}).forEach(([key,count])=>{if(key.trim())all[key]=(all[key]||0)+count});return all},{});const letters=Object.entries(totals).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([key])=>key);openWeakPractice(letters); };
  ...
  const saveResult = (result: Omit<ResultRecord, "id" | "date">) => {
    ...
    if (!result.title.startsWith("✎") && (!result.lessonId || result.accuracy >= requiredAccuracy(result.lessonId))) markComplete(result.lessonId);
  ...
  return <main className="app-shell">
    ...
    {view === "lessons" && <Lessons completed={completed} history={history} openLesson={openLesson} onAdaptive={openAdaptive} onRace={() => navigate("race")} onDictation={() => navigate("dictation")} />}
    ...
    {view === "race" && <RacePage history={history} onComplete={saveResult} onResults={() => navigate("results")} />}
    {view === "dictation" && <DictationPage onComplete={saveResult} onResults={() => navigate("results")} />}
...
function Header({ view, navigate, user, profile, cloudState, onAccount }: { view: View; navigate: (view: View) => void; user: User | null; profile: CloudProfile | null; cloudState: "local" | "syncing" | "synced"; onAccount: () => void }) {
  return <header className="topbar"><button className="brand" onClick={() => navigate("home")} aria-label="QazaqType басты беті"><span>Q</span>azaqType</button>
    <nav>{([['home','Басты бет'],['lessons','Сабақтар'],['trainer','Тренажёр'],['speed','Сынақ'],['race','Жарыс'],['dictation','Диктант'],['results','Нәтижелер']] as [View,string][]).map(([id,label]) => <button key={id} onClick={() => navigate(id)} className={view === id ? "nav-active" : ""}>{label}</button>)}</nav>
...
function Lessons({ completed, history, openLesson, onAdaptive, onRace, onDictation }: { completed: number[]; history: ResultRecord[]; openLesson: (lesson: Lesson) => void; onAdaptive: () => void; onRace: () => void; onDictation: () => void }) {
  const passed = new Set(history.filter(result => result.lessonId && result.accuracy >= requiredAccuracy(result.lessonId)).map(result => result.lessonId as number));
  completed.forEach(id => { if (history.some(result => result.lessonId === id && result.accuracy >= requiredAccuracy(id))) passed.add(id); });
  const percent = Math.round((passed.size / totalLessonCount) * 100);
  const mistakeTotals = history.reduce<Record<string, number>>((all, result) => { Object.entries(result.mistakeKeys || {}).forEach(([key, count]) => { if (key.trim()) all[key] = (all[key] || 0) + count; }); return all; }, {});
  const weakLetters = Object.entries(mistakeTotals).sort((a, b) => b[1] - a[1]).slice(0, 5);
  const nextLesson = lessonGroups.flatMap(group => group.lessons).find(lesson => !passed.has(lesson.id));
  return <section className="catalog page-width"><div className="catalog-head"><div><p className="eyebrow">ОҚУ БАҒДАРЛАМАСЫ</p><h1>Сабақтар</h1><p>Әр сабақтың мақсатын орындап, келесі кезеңді ашыңыз.</p></div><div className="overall-progress"><div className="progress-ring" style={{ "--p": `${percent * 3.6}deg` } as React.CSSProperties}><span>{percent}%</span></div><div><b>{passed.size} / {totalLessonCount}</b><small>сабақ сәтті аяқталды</small></div></div></div>
    <div className="learning-tools"><section className="adaptive-card"><div><span>✦</span><div><p className="eyebrow">БЕЙІМДЕЛЕТІН ЖАТТЫҒУ</p><h2>{weakLetters.length ? "Қателеріңізге арналған жеке сабақ" : "Қазақ әріптерінен бастап көріңіз"}</h2><p>{weakLetters.length ? `Жүйе ең қиын әріптерді анықтады: ${weakLetters.map(([letter]) => letter.toUpperCase()).join(", ")}.` : "Алғашқы нәтижелерден кейін жүйе сізге жеке жаттығу құрады."}</p></div></div><button onClick={onAdaptive}>Жеке жаттығуды бастау →</button></section><section className="next-step"><p className="eyebrow">КЕЛЕСІ ҚАДАМ</p><b>{nextLesson ? `${nextLesson.id}. ${nextLesson.title}` : "Бағдарлама аяқталды"}</b><small>{nextLesson ? `Өту шарты: кемінде ${requiredAccuracy(nextLesson.id)}% дәлдік` : "Барлық сабақ сәтті орындалды"}</small></section></div>
    <div className="mode-cards"><button onClick={onRace}><i>🏁</i><span><b>Апталық жарыс</b><small>Ортақ мәтін · сынып рейтингі</small></span><em>Жарысу →</em></button><button onClick={onDictation}><i>🔊</i><span><b>Дыбыстық диктант</b><small>Тыңдаңыз және естігеніңізді теріңіз</small></span><em>Тыңдау →</em></button></div>
    {lessonGroups.map((group, groupIndex) => <section className="lesson-group" key={group.title}><div className="group-title"><span>{String(groupIndex + 1).padStart(2, "0")}</span><div><h2>{group.title}</h2><p>{group.description}</p></div></div><div className="lesson-grid">{group.lessons.map(lesson => { const done = passed.has(lesson.id), unlocked = lesson.id === 1 || passed.has(lesson.id - 1), attempts = history.filter(result => result.lessonId === lesson.id), best = [...attempts].sort((a, b) => b.accuracy - a.accuracy || b.cpm - a.cpm)[0]; return <button className={`lesson-card ${done ? "completed" : ""} ${!unlocked ? "locked" : ""}`} key={lesson.id} onClick={() => unlocked && openLesson(lesson)} disabled={!unlocked}><div className="card-top"><span>{lesson.level}</span><b>{done ? "✓" : unlocked ? String(lesson.id).padStart(2, "0") : "🔒"}</b></div><div className="card-main"><small>{lesson.subtitle}</small><h3>{lesson.title}</h3><p className="lesson-goal">{!unlocked ? `Алдымен ${lesson.id - 1}-сабақты аяқтаңыз` : lessonTip(lesson.id)}</p><div className="circle-score">{best ? best.cpm : 0}</div></div><div className="card-foot"><span>◎ {best ? best.accuracy : 0}%</span><span>Мақсат {requiredAccuracy(lesson.id)}%</span></div></button>; })}</div></section>)}
  </section>;
}
...
function Trainer({ exercise, onComplete, onChooseAbai, onLessons }: { exercise: { title:string; text:string; lessonId?:number; abaiIndex?:number }; onComplete:(result:Omit<ResultRecord,"id"|"date">)=>void; onChooseAbai:(i:number)=>void; onLessons:()=>void }) {
  ...
  const customExercise = exercise.title.startsWith("✎"), passTarget=exercise.lessonId&&!customExercise?requiredAccuracy(exercise.lessonId):0, passed=!passTarget||accuracy>=passTarget;
  return <section className="workspace trainer-page"><div className="trainer-crumb"><button onClick={onLessons}>← Сабақтар</button><span>/</span><span>{customExercise ? "Мұғалім тапсырмасы" : exercise.lessonId ? "Жаттығу" : "Абайдың қара сөздері"}</span></div>
    ...
    <div className="stats-row"><div><span>Жылдамдық</span><strong>{cpm}</strong><small>таң/мин</small></div><div><span>Дәлдік</span><strong>{accuracy}</strong><small>%</small></div><div><span>Қате</span><strong>{errors}</strong><small>рет</small></div>{passTarget>0&&<div className="pass-target"><span>Өту шарты</span><strong>{passTarget}%</strong><small>дәлдік</small></div>}<div className="live-progress"><span>{Math.round(progress)}% орындалды</span><i><b style={{width:`${progress}%`}}/></i></div></div>
    ...
    {finished&&<div className="modal-backdrop"><section className="result-modal"><button className="close" onClick={()=>setFinished(false)}>×</button><div className="trophy">{passed?"★":"↻"}</div><p className="eyebrow">ЖАТТЫҒУ АЯҚТАЛДЫ</p><h2>{passed?"Сабақ сәтті өтті!":"Тағы бір рет байқап көріңіз"}</h2><p className="result-copy">{passed?"Өту шарты орындалды, келесі сабақ ашылды.":`Келесі сабақты ашу үшін кемінде ${passTarget}% дәлдік қажет.`}</p><div className="result-grid"><div><span>{formatTime(seconds)}</span><small>Уақыт</small></div><div><span>{cpm}</span><small>Таң/мин</small></div><div><span>{accuracy}%</span><small>Дәлдік</small></div></div><div className="modal-actions"><button onClick={reset}>{passed?"Қайта өту":"Қайталап көру"}</button><button className="primary" onClick={exercise.abaiIndex!==undefined?()=>onChooseAbai((exercise.abaiIndex!+1)%abaiTexts.length):onLessons}>{exercise.abaiIndex!==undefined?"Келесі қара сөз →":"Сабақтарға оралу"}</button></div></section></div>}
  </section>;
}
...
function Footer({navigate}:{navigate:(v:View)=>void}){return <footer><div className="page-width"><button className="brand" onClick={()=>navigate("home")}><span>Q</span>azaqType</button><p>Қазақша жылдам әрі сауатты теруге арналған жаттықтырғыш.</p><div><button onClick={()=>navigate("home")}>Басты бет</button><button onClick={()=>navigate("lessons")}>Сабақтар</button><button onClick={()=>navigate("race")}>Жарыс</button><button onClick={()=>navigate("dictation")}>Диктант</button><button onClick={()=>navigate("results")}>Нәтижелер</button></div><small>© 2026 QazaqType · Қазақ тілі үшін жасалды</small></div></footer>}
